...
export function getCurrentTimeInUS(): Date {
  ...
  // Convert the UTC time to US's time
  const offsetEST = -6; // Eastern Standtard time in US (UTC -6)
...
export function formatTimeForUS(date: Date): string {
  ...
  // For simplicity, here I'm just appending "EST" for Eastern Standard Time.
  formattedTime += " EST";
